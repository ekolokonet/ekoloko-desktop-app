...
// per webContents ? there is no per-window volume API ? and the game's audio
// comes from the PPAPI Flash plugin, so scripting HTML5 media volume into the
// page does nothing either. The only lever that actually attenuates Flash
// audio is the OS mixer's per-application volume:
...
// Slider drags fire many updates per second; collapse them before touching
// the OS mixer.
const DEBOUNCE_MS = 150;
// Audio sessions / sink-inputs only exist once the app has started playing
// sound, which can happen *after* the user moved the slider (e.g. adjusting
// volume on the login page before the game loads). Re-apply periodically so
// late-appearing streams pick up the chosen level.
...
// Name the OS shows for our audio sessions: the executable's base name
// ("ekoloko" packaged, "electron" in development).
...
// Enumerates WASAPI audio sessions on the default output device and sets the
// session volume for every session owned by a process with our executable
// name (Chromium plays audio from the browser process or an audio-service
// utility child; both carry the app's exe name). Reads one volume float
// (0..1, invariant culture) per stdin line so a single PowerShell process ?
// and a single Add-Type compile ? serves the whole app session.
...
function windowsHelper() {
  ...
  if (psSpawnFailures >= 3) return null; // powershell keeps failing; stop retrying
  try {
...
function applyLinux(volume) {
  execFile("pactl", ["list", "sink-inputs"], (err, stdout) => {
    ...
    // Match our own audio streams by client PID (Chromium's PulseAudio client
    // lives in the browser process) or, as a fallback, by executable name.
    const ids = [];
    let current = null;
...
function apply(volume) {
  switch (process.platform) {
    ...
    case "linux":
      ...
      break;
    default:
      // macOS: no public per-app volume API. 0% is still covered by the hard
      // mute in index.js; intermediate levels only affect the OS mixer on
      // Windows/Linux.
      break;
  }
}
...
function set(volume) {
  ...
  if (!reapplyTimer && (process.platform === "win32" || process.platform === "linux")) {
    reapplyTimer = setInterval(() => {
      // Only needed while attenuated; at 100% new streams already start there.
...
function dispose() {
  ...
  if (psProc) {
    try {
      psProc.stdin.end(); // helper exits on stdin EOF
      psProc.kill();
    } catch (e) {
      /* already gone */
    }
